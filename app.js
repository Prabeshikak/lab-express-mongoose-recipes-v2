...
app.post("/recipes", async (request, response) => {
  ...
  try {
    ...
    response.status(500).json({
      error,
      message: "Internal Server Error while creating a new recipe",
    });
  }
});
...
app.get("/recipes", async (request, response) => {
  try {
    ...
    response.status(500).json({
      error,
      message: "Internal Server Error while getting all the recipes",
    });
  }
});
...
app.get("/recipes/:id", async (request, response) => {
  try {
    ...
    response.status(500).json({
      error,
      message: "Internal Server Error while getting the recipe",
    });
  }
});
...
app.put("/recipes/:id", async (request, response) => {
  try {
    ...
    response.status(500).json({
      error,
      message: "Internal Server Error while updating the recipe",
    });
  }
});
